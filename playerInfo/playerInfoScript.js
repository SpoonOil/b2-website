...
async function getCurrentSeasonID() {
  const HOMS_URL = "https://data.ninjakiwi.com/battles2/homs/";

  return fetch(HOMS_URL)
    .then((response) => response.json())
    .then((json) => {
      const id = json.body.find((leaderboard) => leaderboard.live).id;
      console.log(id);
      return id;
    });
}

async function fetchMatches(playerUrl) {
  const currentSeasonId = await getCurrentSeasonID();

  const fullUrl =
    "http://116.203.238.44/api/matches_nk?season=" +
    currentSeasonId +
    "&plid=" +
    playerUrl;
...
function displayData(data, playerName) {
  loadouts = [];
  for (let match of data) {
    let player =
      match.playerLeft.displayName === playerName
        ? match.playerLeft
        : match.playerRight;
    let loadout = [player.towerone, player.towertwo, player.towerthree];
    console.log(loadout);
  }
}
...
async function displayResults(json, playerURL) {
  ...
  fetchMatches(url).then((data) => displayData(data, player.displayName));
  // await updateSeasonalWinrate(player, playerURL);
}

function generateBanner(player) {
  const playerBannerContainer = document.querySelector(
...
function updateRankedStats(player) {
  for (const key in player.rankedStats) {
    let dataOutput = document.querySelector(
      `[data-key = ${key.toString()}]>td:last-child`,
    );
...
function getColor(value) {
  ...
  if (value >= 0.5) {
    value = 1 - value;
    hue = ((1 - value) * 150).toString(10);
  } else {
    hue = ((1 - value) * 50).toString(10);
  }
...
function updateSummary(player) {
  ...
  let winrate =
    (player.rankedStats["wins"] /
      (player.rankedStats["wins"] + player.rankedStats["losses"])) *
    100;
  ...
  for (let i = 0; i < towers.length; i++) {
    ...
    if (
      tower == favoriteTower1 ||
      tower == favoriteTower2 ||
      tower == favoriteTower3
    ) {
      continue;
    }
...
function updateGeneralStats() {
  ...
  bloonStats.forEach((bloon) => {
    generateBloonRow(bloon, bloonTable, moabTable);
  });
...
function updateClans(player) {
  ...
  function showClanStats(json) {
    clan = json.body;
    console.log(clan);
    document.getElementById("clanTitle").innerHTML =
      `<h3 class="summaryHeader">Clan Stats</h3>`;
